    ...
    return (
        <div className="flex flex-col min-h-screen bg-[#f8fbff] pb-24 animate-in fade-in duration-500">
            ...
            <div className="bg-white/60 backdrop-blur-xl px-6 py-6 flex items-center gap-4 sticky top-0 z-10 border-b border-white shadow-sm">
                <button onClick={onBack} className="p-2 bg-green-50 text-green-600 rounded-xl hover:scale-110 transition-transform">
                    <ArrowLeft className="w-5 h-5" />
                </button>
                <div>
                    <h2 className="text-2xl font-black text-gray-900 tracking-tight">Mis Pedidos</h2>
                    <p className="text-[10px] font-black text-green-400 uppercase tracking-widest leading-none">Historial de Compras</p>
                </div>
            </div>
            ...
            <div className="flex-1 px-4 py-8 max-w-2xl mx-auto w-full">
                ...
                {orders.length === 0 ? (
                    <div className="text-center py-20 bg-white/60 backdrop-blur-xl rounded-[2.5rem] border border-white shadow-xl shadow-green-500/5">
                        ...
                        <button
                            onClick={onBack}
                            className="bg-green-600 text-white px-10 py-4 rounded-[1.5rem] font-black shadow-xl shadow-green-200 active:scale-95 transition-all text-xs uppercase tracking-widest hover:bg-green-700"
                        >
                            Empezar a Comprar
                        </button>
                    </div>
                    ...
                    <div className="space-y-6">
                        {orders.map((order) => (
                            <div key={order.id} className="group bg-white/60 backdrop-blur-xl rounded-[2.5rem] border border-white shadow-xl hover:shadow-green-500/10 transition-all duration-500 overflow-hidden">
                                    ...
                                    {order.items.map((item, idx) => (
                                        <div key={idx} className="flex justify-between items-center bg-white/40 p-3 rounded-2xl border border-white shadow-sm group-hover:bg-white/60 transition-colors">
                                            <div className="flex items-center gap-4">
                                                <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center font-black text-green-600 border border-green-50 text-xs shadow-sm">
                                                    {item.quantity}x
                                                </div>
                                ...
                                <div className="p-6 bg-green-600/5 flex justify-between items-center">
                                    <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center text-green-500 shadow-sm border border-green-100">
                                            <span className="text-xs">📅</span>
                                        </div>
                                        <div>
                                            <p className="text-[9px] font-black text-green-400 uppercase tracking-widest leading-none mb-1">Fecha</p>
                                    ...
                                    <div className="text-right">
                                        <p className="text-[9px] font-black text-green-400 uppercase tracking-widest mb-1 leading-none">Total Pagado</p>
